...
function Auth() {
  ...
  // Function to store user data in Supabase (without authentication)
  // const storeUserDataInSupabase = async (email: string, fullName: string = "") => {
  //   try {
  //     // Store user info in Supabase profile table (assuming it exists)
  //     // Change 'profiles' to match your actual Supabase table name
  //     const { error } = await supabase
  //       .from('profiles')
  //       .upsert([
  //         { 
  //           email, 
  //           full_name: fullName || email.split('@')[0] // Use fullName if available
  //         }
  //       ], 
  //       { onConflict: 'email' });
  ...
  //     // Set session data in localStorage for Navbar to use
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .upsert([
          ...
          { 
            onConflict: 'email', // Make sure 'email' has a unique constraint in your table
            returning: 'minimal' // Optional: reduces response size
          }
        ]);
  
      if (error) throw error;
      
      // Store email in localStorage for Navbar
      localStorage.setItem('userEmail', email);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store user data in Supabase
      await storeUserDataInSupabase(formData.email, formData.fullName);
